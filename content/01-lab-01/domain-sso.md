# Domain administration using AWS IAM Identity Center
In this lab you create a domain using IAM Identity Center authentication method and learn how to manage user profiles.

Complete this optional lab if you would like to experiment with domain user onboarding and management via the IAM Identity Center.
...
- **Authentication**: change to _AWS IAM Identity Center_
- **Permission**
...
In this lab you're going to implement user federation using IAM Identity Center using Identity Center directory as the [identity source](https://docs.aws.amazon.com/singlesignon/latest/userguide/manage-your-identity-source.html).
...
You can select users or groups from IAM Identity Center which need to be added to the domain. If you assign a user, a new user profile is created automatically immediately. If you assign a group, a new user profile is created first time the user federates in the Studio.

2. **From IAM Identity Center** - you can [assign user access to applications](https://docs.aws.amazon.com/singlesignon/latest/userguide/assignuserstoapp.html) directly in IAM Identity Center. First time a user federates in the Studio, a new user profile is automatically created.

3. **Using the AWS CLI or AWS CloudFormation** - you can use [`CreateUserProfile`](https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_CreateUserProfile.html) API or [`create-user-profile`](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/sagemaker/create-user-profile.html) AWS CLI call to create a new user profile programmatically. You must set the `single-sign-on-user-identifier` to `UserName` and `single-sign-on-user-value` to the username of the corresponding user in IAM Identity Center. For CloudFormation-based creation of user profiles use the[`AWS::SageMaker::UserProfile`](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-sagemaker-userprofile.html) resource.

❗ You must first assign a user or a group to the domain via SageMaker console or IAM identity center before user can federate in the Studio from the AWS access portal.

### Configure user profile
You can use the following two patterns to configure user profile.

**Configuring user profiles up-front**
After domain provisioning, manually create user profiles for IdP federated users based on the IdP or AD group membership and attributes by using [`CreateUserProfile`](https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_CreateUserProfile.html) API. You can configure each profile based on some metadata, for example taken from IdP or other metadata or entitlement store. This approach is not strictly dynamic as everything must be provisioned up-front, including user profiles for the anticipated domain users. You need to develop some IdP sync mechanism for user housekeeping and profile configuration updates based on the changes in the IdP or AD.
...
```
EventBridge will launch this rule every time a new app is created for the user profile. The event's request parameters contain all needed information to identify the app type, the user profile name, and the domain id:
```json
"requestParameters": {
        "domainId": "d-yjd7ee0p9bib",
...
You can update user profile and set the designated execution role and other user-specific settings by calling `UpdateUserProfile` API. Note, that you cannot update the user profile and `UpdateUserProfile` will fail if the JupyterServer app is in `InService` status. The user profile must be updated when there are no user app running or the JupyterServer app isn't in `InService` status.
...
Your workforce identities, more all users which belong to the `sagemaker_datascientist` group, can now access the Studio without need to sign in to the AWS account.
    ...
    "UserProfiles": [
        {
            ...
            "UserProfileName": "sagemaker-developer-1-471",
            "Status": "InService",
...
##### Update user profile
Let's update the user profile and set the execution role.
...
#### Add user to domain via IAM Identity Center
In this section you're going to use IAM Identity Center to assign a group to the domain. You'll also create a new user profile up-front using AWS CLI.

##### Assign group to domain
1. Open the [IAM Identity Center console](https://console.aws.amazon.com/singlesignon) and choose **Applications**.
2. In the list of the configured applications choose the `Amazon SageMaker Studio (<DOMAIN-ID>)`:

![](../../static/img/sso-assign-application.png)




##### Create user profile
Now you create a new user profile in the domain before the user federates in the Studio first time. You're going to use MLOps execution role you provisioned with the CloudFormation stack.
...
Second, create a new user profile in the domain using the following AWS CLI command. Use the username for the `user2-mlops` in the IAM Identity Center, it's `user2-mlops` if you used the proposed user name. Replace the `<STUDIOROLE-MLOPS-ARN>` with the role's ARN:
```sh
...
```
This command creates a new user profile mapped to the specific user identity from the IAM Identity Center. 
...
##### Sign-in to Studio
Clear session cookies in the browser or use another browser before you sign in the AWS access portal. Sign in as the user `user2-mlops`. This user has Amazon SageMaker Studio as an available application in the portal.
